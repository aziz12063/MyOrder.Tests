...
public class BasketActionsJsonRepository(
    ...
    : BaseJsonRepository(eventAggregator, basketService, logger, environment), IBasketActionsRepository
{
    ...
    public async Task<NewBasketResponseDto?> PostNewBasketAsync(Dictionary<string, string?> newBasketRequest, CancellationToken cancellationToken = default)
    {
        ...
        return await ExecuteAsync(
             async (token) =>
             {
                 var response = new NewBasketResponseDto(true, null, null, null);
    ...
    {
        var jsonFilePath = Path.Combine(baseDataPath, "reload.json");
        ...
        return await ExecuteAsync(
             (token) => {
                 return Task.FromResult(JsonConvert.DeserializeObject<NewOrderContextResponse>(json));
             },
            operationDescription,
            cancellationToken);
    }
    ...
    public async Task<ProcedureCallResponseDto?> PostProcedureCallAsync(ImmutableList<string?> readyToPostProcedureCall, CancellationToken cancellationToken = default)
    {
        ...
        return await ExecuteAsync(
            async (token) =>
            {
                var response = new ProcedureCallResponseDto(true, null, null, null, null, null);
