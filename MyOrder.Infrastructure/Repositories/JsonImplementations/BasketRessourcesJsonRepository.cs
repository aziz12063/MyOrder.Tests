...
public class BasketRessourcesJsonRepository(IEventAggregator eventAggregator, IBasketService basketService,
    ...
    : BaseJsonRepository(eventAggregator, basketService, logger, environment), IBasketResourcesRepository
{
    ...
    {
        var jsonFilePath = Path.Combine(baseDataPath, "carrierTypes.json");
        if (!File.Exists(jsonFilePath))
            return null;
    ...
    {
        var jsonFilePath = Path.Combine(baseDataPath, "deliveryCountries.json");
        if (!File.Exists(jsonFilePath))
            return null;
