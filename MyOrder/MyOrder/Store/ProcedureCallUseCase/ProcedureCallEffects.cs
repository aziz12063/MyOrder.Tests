...
namespace MyOrder.Store.ProcedureCallUseCase
{
    ...
    {
        private readonly Dictionary<string, Action<IDispatcher, string>> _refreshCallActions = new Dictionary<string, Action<IDispatcher, string>>
        {
            ...
            IDispatcher dispatcher)
        {
            ...
            foreach (var call in receivedAction.ProcedureCallResponse.RefreshCalls)
            {
                ...
                {
                    logger.LogError($"No action mapped for refresh call: {call}");
                }
            }
        }
    }
